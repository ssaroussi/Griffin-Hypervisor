...
static inline uintptr_t epte_addr(epte_t epte) { return (epte & EPTE_ADDR); }

static inline uintptr_t epte_page_vaddr(epte_t epte) {
  return (uintptr_t)__va(epte_addr(epte));
}

static inline epte_t epte_flags(epte_t epte) { return (epte & EPTE_FLAGS); }

static inline int epte_present(epte_t epte) { return (epte & __EPTE_FULL) > 0; }

static inline int epte_big(epte_t epte) { return (epte & __EPTE_SZ) > 0; }

#define ADDR_INVAL ((unsigned long)-1)

static unsigned long hva_to_gpa(vmx_vcpu_t *vcpu, struct mm_struct *mm,
                                unsigned long hva) {
...
static unsigned long gpa_to_hva(vmx_vcpu_t *vcpu, struct mm_struct *mm,
                                unsigned long gpa) {
  uintptr_t phys_end = (1ULL << boot_cpu_data.x86_phys_bits);
...
static int ept_lookup_gpa(vmx_vcpu_t *vcpu, void *gpa, int level, int create,
                          epte_t **epte_out) {
  int i;
...
static int ept_lookup(vmx_vcpu_t *vcpu, struct mm_struct *mm, void *hva,
...
static int ept_map_apic_page(vmx_vcpu_t *vcpu, int make_write,
                             unsigned long gpa) {
  int ret;
  epte_t *epte, flags;
  ...
  flags = __EPTE_READ | __EPTE_TYPE(EPTE_TYPE_UC) | __EPTE_IPAT | __EPTE_PFNMAP;
  if (make_write)
    flags |= __EPTE_WRITE;
...
static int ept_set_pfnmap_epte(vmx_vcpu_t *vcpu, int make_write,
...
static int ept_set_epte(vmx_vcpu_t *vcpu, int make_write, unsigned long gpa,
                        unsigned long hva) {
  int ret;
  epte_t *epte, flags;
  struct page *page;
...
int vmx_do_ept_fault(vmx_vcpu_t *vcpu, unsigned long gpa, unsigned long gva,
                     int fault_flags) {
  int ret;
 ...
 */
static int ept_invalidate_page(vmx_vcpu_t *vcpu, struct mm_struct *mm,
                               unsigned long addr) {
  int ret;
  epte_t *epte;
 ...
 */
static int ept_check_page_mapped(vmx_vcpu_t *vcpu, struct mm_struct *mm,
                                 unsigned long addr) {
  int ret;
  epte_t *epte;
  void *gpa = (void *)hva_to_gpa(vcpu, mm, (unsigned long)addr);
 ...
 */
static int ept_check_page_accessed(vmx_vcpu_t *vcpu, struct mm_struct *mm,
...
static inline vmx_vcpu_t *mmu_notifier_to_vmx(struct mmu_notifier *mn) {
  return container_of(mn, vmx_vcpu_t, mmu_notifier);
}

void ept_mmu_notifier_invalidate_page(struct mmu_notifier *mn,
                                      struct mm_struct *mm,
                                      unsigned long address) {
  vmx_vcpu_t *vcpu = mmu_notifier_to_vmx(mn);
...
int vmx_init_ept(vmx_vcpu_t *vcpu) {
...
int vmx_create_ept(vmx_vcpu_t *vcpu) {
  int ret;
...
void vmx_destroy_ept(vmx_vcpu_t *vcpu) {
  if (current->mm)
